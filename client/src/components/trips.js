...
const rows = [
    ...
    { id: 'nightsStayed', numeric: true, disablePadding: false, label: 'Nights Stayed' },
...
class Trips extends React.Component {
    ...
    handleSelectAllClick = event => {
        if (event.target.checked) {
            this.setState(state => ({ selected: state.trips.map(n => n.id) }));
            return;
        }
    ...
    render() {
        ...
        const { trips, order, orderBy, selected, rowsPerPage, page } = this.state;
            ...
            <Paper className={classes.root}>
                ...
                <div className={classes.tableWrapper}>
                    <Table className={classes.table} aria-labelledby="tableTitle">
                        ...
                        />
                        <TableBody>
                            {stableSort(this.state.trips, getSorting(order, orderBy))
                                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(trip => {
                                    const isSelected = this.isSelected(trip._id);
                                    return (
                                        <TableRow
                                            hover
                                            onClick={event => this.handleClick(event, trip._id)}
                                            role="checkbox"
